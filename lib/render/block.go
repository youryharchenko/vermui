...
func (b *Block) Align() {
	...
	// float
	b.area = align.AlignArea(align.TermRect(), b.area, b.Float)
	b.area = align.MoveArea(b.area, b.X, b.Y)
...
func (b *Block) RemoveHandler(path string) {
	events.RemoveWgtHandler(b, path)
}

func (b *Block) ClearHandlers(path string) {
	events.RemoveWgtHandler(b, path)
}
